...
inputs:
	input_file_path, line 175, type str: path to the input file # TODO: check line numbers
	output_file_path, line 175, type str: path to the output file (optional)
...
def compute_mrkt_intervals(sample_dataset:list, market_threshold_lower:float = 0.8, 
	...
	# remove instances where the endpoint is None (latest market has not endpoint, because it is still ongoing) #TODO: remove this, its for older version 
	mrkt_intervals['bull_markets'] = all_markets['bull_markets']#[_ for _ in all_markets['bull_markets'] if _[1] is not None]
	mrkt_intervals['bear_markets'] = all_markets['bear_markets']#[_ for _ in all_markets['bear_markets'] if _[1] is not None]
...
def export_intervals_to_csv(sample_dataset, mrkt_intervals, output_file_path):
	...
	for interval in mrkt_intervals['bull_markets']:
		#TODO: if first point in bull markets is the same as the first point of sample_dataset, set column to Bull
		update_column_by_interval(sample_dataset_edit,interval,2,'Bull')
	for interval in mrkt_intervals['bear_markets']:
		#TODO: if first point in bear markets is the same as the first point of sample_dataset, set column to Bull
...
def main(input_file_path,output_file_path,detailed=False):
