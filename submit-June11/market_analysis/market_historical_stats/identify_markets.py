...
outputs:
	return {"bull_markets":bulls, "bear_markets":bears}, line 107, type dict: {"bull_markets" : bull markets as [[date as datetime.datetime, closing price as float], ...], "bear_markets": bear markets as [[date as datetime.datetime, closing price as float], ...]}
...
def identify_markets(data_set: "[[datetime.datetime, float], ...]", 
	...
	# DONE: if first market endpoint is not the same as the first date, add one more market to the opposite type starting from the first point
	...
	# DONE: last market endpoint should be changed from None to the last date
	if bulls[-1][1]==None:
