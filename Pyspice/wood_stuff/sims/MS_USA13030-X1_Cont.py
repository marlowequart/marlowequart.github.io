...
circuit.include(spice_library['ngspice_top']) # Note that PySpice will parse ALL *.lib in the directory, even if they are not included, so keep extra .libs out
circuit.X(1,'ngspice_top', 'a', 'b', 'c', 'angle', 'h1', 'h2', 'h3', 'i_sns_a', 'i_sns_b', 'i_sns_c', 'p_28v', 'target', 'w', 'clk') # Top level circuit annotated as "x1"
...
analysis = simulator.transient(step_time=5E-6, end_time=end_time, max_time=10000E-6, use_initial_condition=True) # strangely, constraining max_time can have opposite the intended effect (in addition to slowing down the sim)
####################################################################################################
