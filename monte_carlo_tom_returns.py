...
def slice_df(df,start_date,end_date):
	# print(df.head(5))
	# print(end_date)
	if df.loc[df['Date'] == start_date].empty:
		start_date_datetime=datetime.datetime.strptime(start_date,"%Y-%m-%d")
		start_date_obj=start_date_datetime - datetime.timedelta(days=1)
...
def start_date_detect_mo_list(df,index_l,index_h,month):
	#return a list of the index of the first day of the desired month over
	...
	#only cycle through the dataframe once. if month is 00, test all months.
...
def main():
	...
	# Use this section for running sims using randomly generated data for market value of SPX
	mrkt_start=1300
	mrkt_avg=0.9
	mrkt_std=4.35
	
	#####
	# Use this section for running the simulation on the actual market data over the given period
	#####
	# Data location for mac:
	# path = '/Users/Marlowe/Marlowe/Securities_Trading/_Ideas/Data/'
	# path = '/Users/Marlowe/gitsite/transfer/TOM/'
	...
	# calculate start date depending on number of months of test
	end_date_data=datetime.datetime(2019,5,10)
	start_date_obj=end_date_data - datetime.timedelta(days=(num_months*365)/12)
	start_date=start_date_obj.strftime("%Y-%m-%d")
	end_date=end_date_data.strftime("%Y-%m-%d")
	#get the low and high index of the dataframe based on the start and end dates
	df_2,idxl,idxh=slice_df(df,start_date,end_date)
	start_idxs=start_date_detect_mo_list(df_2,idxl,idxh,'00')
	start_idxs_m4=[idx+4 for idx in start_idxs]
	start_mrk_val=[df['Open'][idx] for idx in start_idxs_m4]
	market_val=start_mrk_val[::-1]
	
	
	# Generate a list of the value of the index on the start date of each months test
	# use that list as the market value for running sims
	...
	equity_curves_1=[]	#compare option not using actual market values
	all_equity_curves=[]
	for i in range(num_simulations):
		...
		equity_curve_1=[start_equity]
		market_val_1=[mrkt_start]	#comment this to use for std dist generated market vals
		# ~ print()
		# ~ print(i)
		...
		for y in range(num_months):
			...
			kelly_1=kelly_fraction*equity_curve_1[y]*(payoff*win_prob-loss_prob)/payoff
			...
			bet_size_1=kelly_1/(levg_fact*market_val_1[y]*(stop_loss/100))
	...
	losing_sim=0
	cagr=[]
	for index, row in results_df_1.iterrows():
		# Probability of loss, check last column on each row
		if row[num_col] < start_equity:
			losing_sim += 1
	...
	print('Mean CAGR of winning sims using rand mrkt data: '+str(round(np.mean(cagr),1))+', std dev: '+str(round(np.std(cagr),2)))
